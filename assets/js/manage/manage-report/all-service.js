$(function () {
  ...
  $(document).ready(function () {
    if ($(".all-service").length > 0) allService.initialize();
  });
});
...
function drillDownCustomerFormatter(data, type, row, meta) {
  return (
    `
    <a href="` +
    row.customer_url +
    `" target="_blank" class="">`+row.customer+`</a>
  `
  );
}
var allService = (function (self, base) {
  base.action = "ajax_get-all-service";
  base.tableID = "#tb-data";
  base.context = $(".all-service");
  ...
  self.sendEmail = (e) => {
    ...
    Swal.fire({
      ...
      text: "Bạn có chắc muốn gửi thông báo cho các dịch vụ vừa chọn?",
      icon: "question",
      showCancelButton: true,
      confirmButtonText: "Có",
      cancelButtonText: "Không",
      ...
      if (res.value) {
        callAjaxLoading(
          {
            ids: ids,
            action: "ajax_send-email-all-service",
            security: $("#nonce").val(),
          },
          (res) => {
            if (res.success) {
  ...
  base.columnDefs = [
    {
      targets: 0,
      render: drillDownCustomerFormatter,
    },
    {
      targets: 2,
