...
class AjaxPluginType
{
    private $taxonomy = 'plugin-type';
    public function __construct()
    {
        add_action('wp_ajax_ajax_get-list-plugin-type',  array($this, 'func_get_list'));
        add_action('wp_ajax_ajax_delete-list-plugin-type',  array($this, 'func_delete_list'));
        add_action('wp_ajax_ajax_new-plugin-type',  array($this, 'func_new'));
        add_action('wp_ajax_ajax_update-plugin-type',  array($this, 'func_update'));
        add_action('wp_ajax_ajax_delete-plugin-type',  array($this, 'func_delete'));
    }
    public function func_get_list()
    {
        global $CDWFunc;
        // First check the nonce, if it fails the function will break
        check_ajax_referer('ajax-list-plugin-type-nonce', 'security');
        ...
        foreach ($terms as $term) {
            $urlredirect = $CDWFunc->getUrl('detail', 'plugin-type', 'id=' . $term->term_id);
            $id = $term->term_id;
            $name = $term->name;
    ...
    {
        // First check the nonce, if it fails the function will break
        check_ajax_referer('ajax-new-plugin-type-nonce', 'security');
    ...
    {
        // First check the nonce, if it fails the function will break
        check_ajax_referer('ajax-detail-plugin-type-nonce', 'security');
