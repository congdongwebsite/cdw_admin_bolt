...
class AjaxPayment
{
    private $postType = 'payment';
    public function __construct()
    {
        add_action('wp_ajax_ajax_get-list-payment',  array($this, 'func_get_list'));
        add_action('wp_ajax_ajax_delete-list-payment',  array($this, 'func_delete_list'));
        add_action('wp_ajax_ajax_new-payment',  array($this, 'func_new'));
        add_action('wp_ajax_ajax_update-payment',  array($this, 'func_update'));
        add_action('wp_ajax_ajax_delete-payment',  array($this, 'func_delete'));
        add_action('wp_ajax_ajax_load-payment-detail',  array($this, 'func_load_payment_detail'));
        add_action('wp_ajax_ajax_check-payment',  array($this, 'func_check'));
    }
    public function func_get_list()
    {
        global $CDWFunc;
        // First check the nonce, if it fails the function will break
        check_ajax_referer('ajax-list-payment-nonce', 'security');
        ...
        foreach ($posts as $post) {
            $urlredirect = $CDWFunc->getUrl('detail', 'payment', 'id=' . $post->ID);
    ...
    public function func_new()
    {
        global $CDWFunc;
        // First check the nonce, if it fails the function will break
        check_ajax_referer('ajax-new-payment-nonce', 'security');
        ...
        if ($id) {
            ...
            $details = $CDWFunc->wpdb->func_new_detail_post('payment-detail', 'payment-id', $id, $details, $detailColumns);
    ...
    public function func_update()
    {
        global $CDWFunc;
        // First check the nonce, if it fails the function will break
        check_ajax_referer('ajax-detail-payment-nonce', 'security');
        ...
        if ($id) {
            ...
            $details = isset($_POST['details']) ? $_POST['details'] : [];
            $detailColumns = ['note', 'amount'];
            $details = $CDWFunc->wpdb->func_update_detail_post('payment-detail', 'payment-id', $id, $details, $detailColumns);
    ...
    public function func_delete()
    {
        ...
        if (!$check ||  $check == null) {
            ...
            $CDWFunc->wpdb->func_delete_detail_post('payment-detail', 'payment-id', $id);
    ...
    public function func_load_payment_detail()
    {
        global $CDWFunc;
        $postType = 'payment-detail';
        // First check the nonce, if it fails the function will break
        check_ajax_referer('ajax-detail-payment-nonce', 'security');
        ...
        $columns = ['note', 'amount'];
        $data =  $CDWFunc->wpdb->func_load_detail($postType, 'payment-id', $_POST['id'], $columns);
