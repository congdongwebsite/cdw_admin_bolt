<?php
global $CDWUser;
$avatar = $CDWUser->avatar;
$phone = $CDWUser->phone;
$name = $CDWUser->name;
$email = $CDWUser->email;
$birthdate = $CDWUser->birthdate;
$address = $CDWUser->address;
$username = $CDWUser->username;
$useremail = $CDWUser->useremail;
$gender = $CDWUser->gender;
$website = $CDWUser->website;

$dvhc_tp = $CDWUser->idtp;
$dvhc_qh = $CDWUser->idqh;
$dvhc_px = $CDWUser->idpx;
$address = $CDWUser->straddress;

$customer_id = get_user_meta($CDWUser->id, 'customer-id', true);
$kyc_status_meta = get_post_meta($customer_id, 'status-kyc', true);
$phone = get_post_meta($customer_id, 'phone', true);
if (!empty($kyc_status_meta)) {
    $kyc_status = $kyc_status_meta;
}
switch ($kyc_status) {
    case '2':
        $kyc_status_text = 'Đang Xác Thực Tài Khoản';
        $kyc_status_class = 'warning';
        break;
    case '3':
        $kyc_status_text = 'Đã Xác Thực Tài Khoản';
        $kyc_status_class = 'success';
        break;
    case '1':
    default:
        $kyc_status_text = 'Chưa Xác Thực Tài Khoản';
        $kyc_status_class = 'danger';
        break;
}
$is_readonly = ($kyc_status == '3') ? 'readonly' : '';
$is_disabled = ($kyc_status == '3') ? 'disabled' : '';
$cmnd_cccd = '';
$id_card_front_attachment_id = '';
$id_card_back_attachment_id = '';
$id_card_front_url = '';
$id_card_back_url = '';

$company_name = '';
$company_phone = '';
$company_tax_code = '';
$company_address = '';

if ($customer_id) {
    $cmnd_cccd = get_post_meta($customer_id, 'cmnd', true);
    $id_card_front_attachment_id = get_post_meta($customer_id, 'id_card_front', true);
    $id_card_back_attachment_id = get_post_meta($customer_id, 'id_card_back', true);

    if ($id_card_front_attachment_id) {
        $id_card_front_url = wp_get_attachment_url($id_card_front_attachment_id);
    }
    if ($id_card_back_attachment_id) {
        $id_card_back_url = wp_get_attachment_url($id_card_back_attachment_id);
    }

    $company_name = get_post_meta($customer_id, 'company_name', true);
    $company_phone = get_post_meta($customer_id, 'company_phone', true);
    $company_tax_code = get_post_meta($customer_id, 'mst', true);
    $company_address = get_post_meta($customer_id, 'company_address', true);
}
?>
<div class="row clearfix">
    <div class="col-12">
        <div class="card">
            <div class="body">
                <h6 class="mb-0">Trạng thái tài khoản: <span class="text-<?php echo $kyc_status_class; ?> font-weight-bold"><?php echo $kyc_status_text; ?></span></h6>
            </div>
        </div>
    </div>
    <div class="col-lg-12">
        <div class="card">
            <div class="body">
                <ul class="nav nav-tabs-new2">
                    <li class="nav-item"><a class="nav-link active" data-toggle="tab" href="#General">Cài đặt chung</a></li>
                    <li class="nav-item"><a class="nav-link" data-toggle="tab" href="#Settings">Bảo mật</a></li>
                </ul>
            </div>
            <div class="tab-content">
                <div class="tab-pane active" id="General">
                    <form class="form-setting-base-small" method="post">
                        <?php wp_nonce_field('ajax-setting-base-nonce', 'nonce'); ?>
                        <div class="body">

                            <div class="row clearfix">
                                <div class="col-lg-3 col-md-3 col-sm-12 col-12">
                                    <h6>Ảnh đại diện</h6>
                                    <div class="media  d-flex flex-column">
                                        <div class="media-left m-r-15 shadow-sm p-1 mb-0 bg-white rounded">
                                            <img class="avatar" style="width: 100%;height: min-content;" src="<?php echo $avatar; ?>" class="user-photo media-object" alt="<?php echo $name; ?>">
                                        </div>
                                        <div class="media-body">
                                            <p>Tải lên ảnh của bạn.
                                                <br> <em>Ảnh JPG, PNG, JPEG, GIF</em>
                                            </p>
                                            <button type="button" class="btn btn-default" id="btn-upload-photo">Ảnh tải lên</button>
                                            <input type="file" id="avatar-custom" accept="image/*" class="sr-only">
                                        </div>
                                    </div>
                                </div>

                                <div class="col-lg-9 col-md-9 col-sm-12 col-12">
                                    <div class="row clearfix">
                                        <div class="col-lg-6 col-md-6 col-sm-12 col-12">
                                            <div class="row clearfix">
                                                <div class="col-lg-12 col-md-12 col-sm-12 col-12">
                                                    <div class="form-group">
                                                        <label for="first-name" class="control-label">Họ và tên <small class="text-danger">*</small></label>
                                                        <input type="text" id="first-name" name="first-name" value="<?php echo $name; ?>" class="form-control" placeholder="Họ và tên" required <?php echo $is_readonly; ?>>
                                                    </div>
                                                </div>

                                                <div class="col-xl-7 col-lg-6 col-md-12 col-sm-12 col-12">
                                                    <div class="form-group">
                                                        <label for="phone" class="control-label">Số điện thoại <small class="text-danger">*</small></label>
                                                        <input type="text" id="phone" name="phone" class="form-control" value="<?php echo $phone; ?>" placeholder="Số điện thoại" required <?php echo $is_readonly; ?>>
                                                    </div>
                                                </div>
                                                <div class="col-xl-5 col-lg-6  col-md-12 col-sm-12 col-12">
                                                    <div class="form-group">
                                                        <label for="first-name" class="control-label">Giới tính</label>
                                                        <div>
                                                            <label class="fancy-radio">
                                                                <input id="gender" name="gender" value="male" type="radio" <?php echo $gender == 'male' ? 'checked' : ''; ?> <?php echo $is_disabled; ?>>
                                                                <span><i></i>Nam</span>
                                                            </label>
                                                            <label class="fancy-radio">
                                                                <input id="gender" name="gender" value="female" type="radio" <?php echo $gender == 'female' ? 'checked' : ''; ?> <?php echo $is_disabled; ?>>
                                                                <span><i></i>Nữ</span>
                                                            </label>
                                                        </div>
                                                    </div>
                                                </div>

                                                <div class="col-xl-7 col-lg-6 col-md-12 col-sm-12 col-12">
                                                    <div class="form-group">
                                                        <label for="first-name" class="control-label">Website</label>
                                                        <input type="url" id="website" name="website" value="<?php echo $website; ?>" class="form-control" placeholder="http://" <?php echo $is_readonly; ?>>
                                                    </div>
                                                </div>
                                                <div class="col-xl-5 col-lg-6 col-md-12 col-sm-12 col-12">
                                                    <div class="form-group">
                                                        <label for="first-name" class="control-label">Ngày sinh</label>

                                                        <div class="input-group">
                                                            <div class="input-group-prepend">
                                                                <span class="input-group-text"><i class="icon-calendar"></i></span>
                                                            </div>
                                                            <input class="form-control datepicker" id="birthdate" name="birthdate" value="<?php echo $birthdate; ?>" class="form-control" placeholder="Ngày sinh" <?php echo $is_disabled; ?>>
                                                        </div>
                                                    </div>
                                                </div>

                                            </div>
                                        </div>

                                        <div class="col-lg-6 col-md-6 col-sm-12 col-12">
                                            <div class="form-group">
                                                <label for="dvhc-tp" class="control-label">Tỉnh/Thành phố <small class="text-danger">*</small></label>
                                                <select id='dvhc-tp' name='dvhc-tp' class='select2 form-control' data-value="<?php echo $dvhc_tp; ?>" <?php echo $is_disabled; ?> required></select>
                                            </div>

                                            <div class="form-group">
                                                <label for="dvhc-px" class="control-label">Phường/Xã <small class="text-danger">*</small></label>
                                                <select id='dvhc-px' name='dvhc-px' class='select2 form-control' data-value="<?php echo $dvhc_px; ?>" <?php echo $is_disabled; ?> required></select>
                                            </div>
                                            <div class="form-group">
                                                <label for="address" class="control-label">Địa chỉ <small class="text-danger">*</small></label>
                                                <input type="text" id="address" name="address" value="<?php echo $address; ?>" class="form-control" placeholder="Địa chỉ" <?php echo $is_readonly; ?> required>
                                            </div>
                                        </div>

                                        <div class="col-lg-12 col-md-12 col-sm-12 col-12">
                                            <div class="row clearfix">
                                                <div class="col-lg-12 col-md-12 col-sm-12 col-12">
                                                    <div class="form-group">
                                                        <label for="cmnd_cccd" class="control-label">CMND/CCCD <small class="text-danger">*</small></label>
                                                        <input type="text" id="cmnd_cccd" name="cmnd_cccd" value="<?php echo $cmnd_cccd; ?>" class="form-control" placeholder="Số CMND/CCCD" <?php echo $is_readonly; ?> required maxlength="20" pattern="\d{1,20}" data-parsley-pattern-message="Số CMND/CCCD phải bao gồm 20 ký tự số." inputmode="numeric">
                                                    </div>
                                                </div>
                                                <div class="col-lg-6 col-md-6 col-sm-6 col-12">
                                                    <div class="form-group">
                                                        <label class="control-label">Ảnh CMND/CCCD mặt trước <small class="text-danger">*</small></label>
                                                        <div class="media d-flex flex-column">
                                                            <div class="media-left m-r-15">
                                                                <img class="id-card-front-preview" src="<?php echo $id_card_front_url ? $id_card_front_url : 'data:image/png;base64,'; ?>" class="user-photo media-object" alt="Mặt trước CMND/CCCD" width="150">
                                                            </div>

                                                            <?php if ($kyc_status != '3') : ?>
                                                                <div class="media-body">
                                                                    <p>Tải lên ảnh mặt trước CMND/CCCD.
                                                                        <br> <em>Ảnh JPG, PNG, JPEG, GIF</em>
                                                                    </p>
                                                                    <button type="button" class="btn btn-default" id="btn-upload-id-card-front">Tải ảnh lên</button>
                                                                    <input type="file" id="id-card-front-custom" name="id_card_front" accept="image/*" <?php echo $id_card_front_url ?  "": "required" ?> class="sr-only">
                                                                </div>
                                                            <?php endif; ?>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="col-lg-6 col-md-6 col-sm-6 col-12">
                                                    <div class="form-group">
                                                        <label class="control-label">Ảnh CMND/CCCD mặt sau <small class="text-danger">*</small></label>
                                                        <div class="media d-flex flex-column">
                                                            <div class="media-left m-r-15 ">
                                                                <img class="id-card-back-preview" src="<?php echo $id_card_back_url ? $id_card_back_url : 'data:image/png;base64,'; ?>" class="user-photo media-object" alt="Mặt sau CMND/CCCD" width="150">
                                                            </div>

                                                            <?php if ($kyc_status != '3') : ?>
                                                                <div class="media-body">
                                                                    <p>Tải lên ảnh mặt sau CMND/CCCD.
                                                                        <br> <em>Ảnh JPG, PNG, JPEG, GIF</em>
                                                                    </p>
                                                                    <button type="button" class="btn btn-default" id="btn-upload-id-card-back">Tải ảnh lên</button>
                                                                    <input type="file" id="id-card-back-custom" name="id_card_back" accept="image/*" <?php echo $id_card_front_url ?  "": "required" ?> class="sr-only">
                                                                </div>
                                                            <?php endif; ?>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>

                                        <div class="col-lg-12 col-md-12 col-sm-12 col-12 mt-5">
                                            <h6>Thông tin công ty (nếu có)</h6>
                                            <div class="row clearfix">
                                                <div class="col-lg-8 col-md-8 col-sm-12 col-12">
                                                    <div class="form-group">
                                                        <label for="company_name" class="control-label">Tên pháp danh</label>
                                                        <input type="text" id="company_name" name="company_name" value="<?php echo $company_name; ?>" class="form-control" placeholder="Tên pháp danh công ty" <?php echo $is_readonly; ?>>
                                                    </div>
                                                </div>
                                                <div class="col-lg-4 col-md-4 col-sm-6 col-12">
                                                    <div class="form-group">
                                                        <label for="company_phone" class="control-label">Điện thoại công ty</label>
                                                        <input type="text" id="company_phone" name="company_phone" value="<?php echo $company_phone; ?>" class="form-control" placeholder="Số điện thoại công ty" <?php echo $is_readonly; ?>>
                                                    </div>
                                                </div>
                                                <div class="col-lg-4 col-md-4 col-sm-6 col-12">

                                                    <div class="form-group">
                                                        <label for="company_tax_code" class="control-label">Mã số thuế</label>
                                                        <input type="text" id="company_tax_code" name="company_tax_code" value="<?php echo $company_tax_code; ?>" class="form-control" placeholder="Mã số thuế công ty" <?php echo $is_readonly; ?>>
                                                    </div>
                                                </div>
                                                <div class="col-lg-8 col-md-8 col-sm-12 col-12">
                                                    <div class="form-group">
                                                        <label for="company_address" class="control-label">Địa chỉ công ty</label>
                                                        <input type="text" id="company_address" name="company_address" value="<?php echo $company_address; ?>" class="form-control" placeholder="Địa chỉ công ty" <?php echo $is_readonly; ?>>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>

                        </div>
                        <div class="body">

                            <button type="button" class="btn btn-primary btn-setting-base">Cập nhật</button>

                        </div>
                    </form>
                </div>

                <div class="tab-pane" id="Settings">
                    <form class="form-setting-account-small" method="post">
                        <?php wp_nonce_field('ajax-setting-account-nonce', 'nonce'); ?>

                        <div class="body">
                            <div class="row clearfix">
                                <div class="col-lg-6 col-md-12">
                                    <div class="row clearfix">
                                        <div class="col-lg-12 col-md-12 col-sm-12 col-12">
                                            <div class="form-group">
                                                <label for="username" class="control-label">Tài khoản <small class="text-danger">*</small></label>
                                                <input type="text" id="username" name="username" class="form-control" value="<?php echo $username; ?>" disabled placeholder="Tài khoản">
                                            </div>
                                        </div>
                                        <div class="col-lg-12 col-md-12 col-sm-12 col-12">
                                            <div class="form-group">
                                                <label for="email" class="control-label">Email <small class="text-danger">*</small></label>
                                                <input type="email" id="email" name="email" class="form-control" value="<?php echo $email; ?>" placeholder="Email" required <?php echo $is_readonly; ?>>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-lg-6 col-md-12">
                                    <h6>Đổi mật khẩu</h6>
                                    <div class="form-group">
                                        <input type="password" id="current-password" name="current-password" class="form-control" placeholder="Mật khẩu hiện tại">
                                    </div>
                                    <div class="form-group">
                                        <input type="password" id="new-password" name="new-password" class="form-control" placeholder="Mật khẩu mới" data-parsley-pattern="(?=.*[A-Z])(?=.*[!@#$&*])(?=.*[0-9])(?=.*[a-z]).{8}.*" data-parsley-pattern-message="Mật khẩu phải bao gồm: Chữ hoa, ký tự đặc biệt, số, chữ thường, hơn 8 ký tự.">
                                    </div>
                                    <div class="form-group">
                                        <input type="password" id="confirm-password" name="confirm-password" class="form-control" placeholder="Nhập lại mật khẩu" data-parsley-pattern="(?=.*[A-Z])(?=.*[!@#$&*])(?=.*[0-9])(?=.*[a-z]).{8}.*" data-parsley-pattern-message="Mật khẩu phải bao gồm: Chữ hoa, ký tự đặc biệt, số, chữ thường, hơn 8 ký tự.">
                                    </div>
                                </div>
                            </div>
                            <button type="button" class="btn btn-primary btn-setting-account">Cập nhật</button>
                        </div>
                    </form>
                </div>
            </div>
        </div>
    </div>
</div>