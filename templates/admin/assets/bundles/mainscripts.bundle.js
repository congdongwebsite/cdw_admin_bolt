...
var toggleSwitch = document.querySelector(
  ...
  rtl_modeSwitch = document.querySelector('.theme-rtl input[type="checkbox"]'),
  radioFontEL = $(".font_setting input:radio"),
  currentTheme = localStorage.getItem("theme"),
  rtl_mode = localStorage.getItem("rtl_mode"),
...
function switchRTL(e) {
  e.target.checked
    ? ($("body").addClass("rtl_mode"),
      localStorage.setItem("rtl_mode", "rtl_mode"))
    : ($("body").removeClass("rtl_mode"), localStorage.setItem("rtl_mode", ""));
}
function radioFont(e) {
  $("body").removeClass(fontRemove),
...
function skinChanger() {
  ...
  $(".choose-skin li").removeClass("active"),
    ...
    localStorage.setItem("theme-color", "theme-" + t.data("theme"));
}
theme_color &&
  ($(".choose-skin li").removeClass("active"),
  $(".choose-skin li[data-theme='" + theme_color + "']").addClass("active"),
  $("#wrapper").removeClass().addClass(theme_color)),
  $(".choose-skin li").on("click", skinChanger);

var fontRemove = radioFontEL
  .map(function () {
    return this.value;
  })
  .get()
...
font &&
  ($("body").removeClass(fontRemove),
  $("body").addClass(font),
  $(".font_setting input:radio[value='" + font + "']").prop("checked", true));
radioFontEL.on("change", radioFont);

rtl_mode &&
  ($("body").addClass(rtl_mode),
  "rtl_mode" === rtl_mode && (rtl_modeSwitch.checked = !0));
rtl_modeSwitch.addEventListener("change", switchRTL, !1);
