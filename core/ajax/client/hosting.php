...
class AjaxClientHosting
{
    public function __construct()
    {
        add_action('wp_ajax_ajax_get-client-hosting',  array($this, 'func_get_list'));
    }
    ...
    public function func_get_list()
    {
        ...
        check_ajax_referer('ajax-client-hosting-nonce', 'security');

        $columns = ['ip', 'port', 'user', 'pass',  'type', 'price', 'buy_date', 'expiry_date'];
        $fieldSearch = ['ip', 'port'];
        ...
        foreach ($posts as $post) {
            ...
            $cpu = get_post_meta($item["type"], "cpu", true);
            $ram = get_post_meta($item["type"], "ram", true);
            $hhd = get_post_meta($item["type"], "hhd", true);
            $item["cpu"] = $cpu == -1 ? '<small>Tùy chỉnh</small> ' : $cpu;
            $item["ram"] = $ram == -1 ? '<small>Tùy chỉnh</small> ' : $ram;
            $item["hhd"] = $hhd == -1 ? '<small>Tùy chỉnh</small> ' : $hhd;
            ...
            $item["ip"] .= '<br><small class="text-primary">CPU: ' . $item["cpu"] . '- RAM:  ' . $item["ram"] . '- HHD:  ' . $item["hhd"] . '</small>';
            $item["user"] = '<small class="text-primary">User: ' . $item["user"] . '<br>Pass:  ' . $item["pass"] .'</small>';
            $item['action'] = '';
            $data[] = $item;
        }
