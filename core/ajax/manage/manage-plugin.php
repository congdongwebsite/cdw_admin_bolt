...
class AjaxManagePlugin
{
    private $postType = 'plugin';
    public function __construct()
    {
        ...
        add_action('wp_ajax_ajax_update-manage-plugin',  array($this, 'func_update'));
        add_action('wp_ajax_ajax_update-image-manage-plugin',  array($this, 'func_update_image'));
        add_action('wp_ajax_ajax_update-thumbnail-manage-plugin',  array($this, 'func_update_thumbnail'));
        add_action('wp_ajax_ajax_delete-manage-plugin',  array($this, 'func_delete'));
    }
    public function func_get_list()
    {
        global $CDWFunc;
        // First check the nonce, if it fails the function will break
        check_ajax_referer('ajax-list-manage-plugin-nonce', 'security');
        ...
        foreach ($posts as $post) {
            $urlredirect = $CDWFunc->getUrl('detail', 'manage-plugin', 'id=' . $post->ID);
            $id = $post->ID;
    ...
    {
        // First check the nonce, if it fails the function will break
        check_ajax_referer('ajax-new-manage-plugin-nonce', 'security');
        ...
        if (count($type) == 0) {
            wp_send_json_error(['msg' => 'Vui lòng chọn loại plugin']);
        }
    ...
    {
        // First check the nonce, if it fails the function will break
        check_ajax_referer('ajax-detail-manage-plugin-nonce', 'security');
        if (!post_type_exists($this->postType)) {
            wp_send_json_error(['msg' => 'Không tìm thấy loại bài đăng: ' . $this->postType]);
        }
        if (!isset($_POST['id'])) {
